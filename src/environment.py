...
def create_example_environment(env_type: str = "directed_locomotion") -> BrittleStarUndirectedLocomotionEnvironmentConfiguration | BrittleStarDirectedLocomotionEnvironmentConfiguration | BrittleStarLightEscapeEnvironmentConfiguration:
...
def initialize_simulation(env_type="directed_locomotion", num_arms=5, num_segments_per_arm=4, backend="MJC"):
...
def run_simulation(env, state, environment_configuration):
    ...
    processed_frame = render.post_render(render_output=frame, environment_configuration=environment_configuration)
    ...
    while not (state.terminated | state.truncated):
        ...
        processed_frame = render.post_render(env.render(state=state), environment_configuration)
        mjc_frames.append(processed_frame)
...
if __name__ == "__main__":
    ...
    initial_frame = render.post_render(env.render(state=state), environment_configuration)
